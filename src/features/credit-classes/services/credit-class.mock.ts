...
function ensureSessionStore(sectionId: string): LopTinChiBuoiHoc[] {
  ...
  const data: LopTinChiBuoiHoc[] = (section.schedules ?? []).map(
    (schedule, index) => ({
...
export const creditClassMock: ICreditClassService = {
  async list({
    ...
    page = 1,
    perPage = 10,
  }: CreditClassFilter): Promise<PaginatedResult<LopTinChi>> {
    await delay();
  ...
  async create(dto: CreateLopTinChiDto): Promise<LopTinChi> {
    ...
    const semester =
      dto.startDate.slice(0, 4) +
      "-" +
      dto.endDate.slice(0, 4) +
      (new Date(dto.startDate).getMonth() + 1 <= 6 ? ".1" : ".2");
  ...
  async listStudents(
    ...
    const filtered = mockSinhVien
      ...
      .filter((sv) => {
        ...
        return [sv.maSV, sv.hoTen, sv.lop]
          .join(" ")
          .toLowerCase()
          .includes(keyword);
      })
  ...
  async removeStudent(sectionId: string, studentId: string): Promise<void> {
    await delay(150);
    ENROLLMENTS[sectionId] = (ENROLLMENTS[sectionId] ?? []).filter(
      (id) => id !== studentId,
    );
  },
  ...
  async updateSession(
    ...
    sessions[idx] = {
      ...
      statusLabel,
      note: dto.note ?? "",
      roomId: dto.roomId ?? sessions[idx].roomId,
      roomName: dto.roomId
        ? (MOCK_ROOMS.find((room) => room.id === dto.roomId)?.name ??
          sessions[idx].roomName)
        : sessions[idx].roomName,
    };
    return sessions[idx];
  },
