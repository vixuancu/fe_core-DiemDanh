...
export const reportMock: IReportService = {
  ...
  async getOverview(): Promise<ReportOverviewResponse> {
    await delay();
    return {
      student_total: mockSinhVien.length,
      lecturer_total: mockGiangVien.length,
      course_section_total: mockLopTinChi.length,
      camera_total: mockCamera.length,
      camera_online: mockCamera.filter((camera) => camera.cameraStatus === 1)
        .length,
      room_total: mockPhongHoc.length,
      attendance_total: mockDiemDanh.length,
      attendance_present: mockDiemDanh.filter(
        (item) => item.trangThai === "co_mat",
      ).length,
      attendance_late: mockDiemDanh.filter((item) => item.trangThai === "tre")
        .length,
      attendance_absent: mockDiemDanh.filter(
        (item) => item.trangThai === "vang",
      ).length,
    };
  },
  ...
  async getClassSummary(filter: ReportFilter): Promise<ClassSummaryItem[]> {
    ...
    return [
      {
        course_section_id: 1,
        course_section_name: "CS1",
        course_name: "Toán cao cấp",
        attendance_rate: 85.5,
      },
      {
        course_section_id: 2,
        course_section_name: "CS2",
        course_name: "Lập trình Web",
        attendance_rate: 92.0,
      },
      {
        course_section_id: 3,
        course_section_name: "CS3",
  ...
  async getDetails(
    filter: ReportFilter,
  ): Promise<PaginatedResult<ReportDetailItem>> {
    await delay();
