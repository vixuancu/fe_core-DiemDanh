  ...
  const lessons = useMemo(() => {
    ...
    data.data.forEach((lop) => {
      ...
      schedules.forEach((schedule, idx) => {
        ...
        const tietKetThuc = (schedule.endPeriod ?? (schedule.startPeriod + schedule.numberOfPeriods - 1));
    ...
    if (isGiangVien && user?.id) {
      filtered = filtered.filter((item) => item.giangVienId === String(user.id));
    }
  ...
  const dsPhong = useMemo(() => {
    ...
    lessons.forEach((item) => {
      if (!map.has(item.phongHocId)) {
        map.set(item.phongHocId, { id: item.phongHocId, tenPhong: item.tenPhong });
      }
    });
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            <div className="flex items-center justify-center py-16 text-muted-foreground">
              ...
              <span className="ml-2 font-medium">Đang tải thời khóa biểu...</span>
            </div>
          ) : isGiangVien ? (
              ...
              <tbody>
                {caHocs.map((ca) => (
                  <tr key={ca}>
                    ...
                    {daysOfWeek.map((thu) => {
                      const dayLessons = lessons.filter((l) => l.thu === thu && l.caHoc === ca);
                      return (
                        <td key={thu} className="border border-border p-2 align-top bg-white min-h-[140px]">
                          {dayLessons.map((lesson) => (
                            <div key={lesson.id} className="bg-[#f0f9ff]/5 border border-[#009dd9]/20 shadow-sm rounded-xl p-3 mb-2 last:mb-0">
                              <p className="text-[13px] font-bold text-slate-700 text-center break-words mb-2">{lesson.tenMonHoc}</p>
                              <p className="text-[11px] text-slate-500 text-center">Tiết: <span className="text-slate-600">{lesson.tietBatDau}-{lesson.tietKetThuc}</span></p>
                              <p className="text-[11px] text-slate-500 text-center break-all">Mã lớp: <span className="text-slate-600">{lesson.maLop}</span></p>
                              <p className="text-[11px] text-slate-500 text-center mt-1">Phòng: <span className="text-slate-600">{lesson.tenPhong}</span></p>
                            </div>
                          ))}
                        </td>
                      );
            ...
            <table className="w-full text-sm border-collapse table-fixed min-w-[900px]">
              ...
              <tbody>
                {dsPhong.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="border border-border py-8 text-center text-muted-foreground">Không có buổi học trong tuần đã chọn</td>
                  </tr>
                ) : dsPhong.map((phong) => (
                  <tr key={phong.id}>
                    <td className="border border-border py-4 px-2 text-center align-middle font-medium text-xs text-slate-700 bg-slate-50/10">{phong.tenPhong}</td>
                    {daysOfWeek.map((thu) => {
                      const dayLessons = lessons.filter((l) => l.thu === thu && l.phongHocId === phong.id);
                      return (
                        <td key={thu} className="border border-border py-2 px-2 align-top bg-white min-h-[140px]">
                          {dayLessons.map((lesson) => (
                            <div key={lesson.id} className="bg-[#f0f9ff]/50 border border-[#009dd9]/20 rounded-xl p-3 mb-2 last:mb-0 mx-auto w-full max-w-[95%]">
                              <p className="text-[13px] font-bold text-slate-700 text-center break-words mb-2">{lesson.tenMonHoc}</p>
                              <p className="text-[11px] text-slate-500 text-center">Tiết: <span className="text-slate-600">{lesson.tietBatDau}-{lesson.tietKetThuc}</span></p>
                              <p className="text-[11px] text-slate-500 text-center break-all">Mã lớp: <span className="text-slate-600">{lesson.maLop}</span></p>
                              <p className="text-[11px] text-slate-500 text-center mt-1">GV: <span className="text-slate-600">{lesson.tenGiangVien}</span></p>
                            </div>
                          ))}
                        </td>
                      );
