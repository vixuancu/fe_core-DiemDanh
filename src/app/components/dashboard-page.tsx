...
import { mockSinhVien, mockGiangVien, mockCamera } from './data'; // Giữ lại mock cho số lượng SV/GV
...
export function DashboardPage() {
  ...
  const { data: creditClassesData, isLoading: isLoadingClasses } = useCreditClasses({
  ...
  const { data: roomsData, isLoading: isLoadingRooms } = useRooms({ perPage: 100 });
  const rooms = roomsData?.data ?? [];

  const isLoading = isLoadingClasses || isLoadingAtnd || isLoadingSched || isLoadingRooms;
  ...
  const pieData = useMemo(() => [
    { name: 'Có mặt', value: attendances.filter(d => d.trangThai === 'co_mat').length, color: '#22c55e' },
    { name: 'Đi trễ', value: attendances.filter(d => d.trangThai === 'tre').length, color: '#eab308' },
    { name: 'Vắng', value: attendances.filter(d => d.trangThai === 'vang').length, color: '#ef4444' },
  ], [attendances]);
  ...
  const barData = useMemo(() => {
    return myClasses.map(lop => {
      ...
      return {
        name: lop.tenMonHoc.length > 12 ? lop.tenMonHoc.substring(0, 12) + '...' : lop.tenMonHoc,
  ...
  if (isGiangVien) {
    ...
    const checkedInCount = new Set(attendances.filter(a => a.ngay === todayStr).map(a => a.lichHocId)).size;
    ...
    return (
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-xl p-5 border border-border">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <p className="text-sm text-muted-foreground">Lịch dạy hôm nay</p>
          ...
          <div className="bg-white rounded-xl p-5 border border-border">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <p className="text-sm text-muted-foreground">Đã điểm danh (hôm nay)</p>
                <p className="text-2xl">{checkedInCount} / {scheduleToday.length} buổi</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-xl p-5 border border-border">
        ...
        <div className="bg-white rounded-xl p-5 border border-border">
          ...
          {mySchedule.length === 0 ? (
            <p className="text-sm text-muted-foreground">Không có lịch dạy nào.</p>
          ) : (
            <div className="space-y-3">
              {mySchedule.slice(0, 5).map(lich => (
                <div key={lich.id} className="flex items-center gap-4 p-3 rounded-lg bg-muted/50 border border-border">
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <p className="text-xs text-muted-foreground">Tiết {lich.tietBatDau}-{lich.tietKetThuc}</p>
                  </div>
                </div>
              ))}
            </div>
  ...
  const statCards = [
    { label: 'Sinh viên', value: mockSinhVien.length, icon: <GraduationCap className="w-6 h-6" />, color: 'bg-blue-500' },
    { label: 'Giảng viên', value: mockGiangVien.length, icon: <Users className="w-6 h-6" />, color: 'bg-green-500' },
    { label: 'Lớp tín chỉ', value: myClasses.length, icon: <BookOpen className="w-6 h-6" />, color: 'bg-purple-500' },
    ...(user?.role === 'admin' ? [
      { label: 'Camera', value: mockCamera.filter(c => c.trangThai === 'online').length + '/' + mockCamera.length, icon: <Camera className="w-6 h-6" />, color: 'bg-orange-500' },
    ] : [
      { label: 'Phòng học', value: rooms.length, icon: <Camera className="w-6 h-6" />, color: 'bg-orange-500' },
    ]),
  ];
  ...
  return (
    <div>
          ...
          <div className="lg:col-span-2 bg-white rounded-xl p-5 border border-border">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => scrollChart(-1)} className="p-1.5 rounded-lg hover:bg-muted transition cursor-pointer">
            ...
            <div ref={chartScrollRef} className="overflow-x-auto" style={{ scrollbarWidth: 'thin' }}>
              <div style={{ width: chartWidth, minWidth: '100%' }}>
                <ResponsiveContainer width="100%" height={280}>
                  <BarChart data={barData}>
                    ...
                    <Legend />
                    <Bar dataKey="coMat" name="Có mặt" fill="#22c55e" radius={[4, 4, 0, 0]} />
                    <Bar dataKey="tre" name="Đi trễ" fill="#eab308" radius={[4, 4, 0, 0]} />
                    <Bar dataKey="vang" name="Vắng" fill="#ef4444" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
      ...
      <div className="bg-white rounded-xl p-5 border border-border">
        ...
        {attendances.length === 0 ? (
           <p className="text-sm text-muted-foreground">Chưa có bản ghi điểm danh nào.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left py-3 px-4 text-muted-foreground font-normal">Mã SV</th>
                  <th className="text-left py-3 px-4 text-muted-foreground font-normal">Họ tên</th>
                  <th className="text-left py-3 px-4 text-muted-foreground font-normal">Mã lớp tín chỉ</th>
                  <th className="text-left py-3 px-4 text-muted-foreground font-normal">Ngày</th>
                  <th className="text-left py-3 px-4 text-muted-foreground font-normal">Thời gian</th>
                  <th className="text-left py-3 px-4 text-muted-foreground font-normal">Trạng thái</th>
                </tr>
              </thead>
              <tbody>
                {attendances.slice(0, 6).map(dd => (
                  <tr key={dd.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                    ...
                    <td className="py-3 px-4">
                      <span className="text-xs">{dd.tenMonHoc || dd.maLop}</span>
                    </td>
                    <td className="py-3 px-4">{dd.ngay}</td>
                    <td className="py-3 px-4">{dd.thoiGian || '-'}</td>
                    <td className="py-3 px-4">
                      <span className={`px-2 py-1 rounded text-xs ${
                        dd.trangThai === 'co_mat' ? 'bg-green-100 text-green-700' :
                        dd.trangThai === 'tre' ? 'bg-yellow-100 text-yellow-700' :
